...
class LevelTest < ApplicationRecord
  ...
  def grade(answers)
    ...
    per_section  = sections.each_with_index.map do |sec, si|
      ...
      (sec["questions"] || []).each do |q|
        qid = q["id"].to_i
        if given[[ si, qid ]] == q["answer_index"].to_i
          correct     += 1
          sec_correct += 1
        end
      end
  ...
  # Sections annotated with the answer the user gave, for the review screen.
  # `your_answer_index` is nil when the question was left unanswered — the
  # review UI relies on that to tell "skipped" apart from "picked option A".
  def sections_with_answers(answers)
    given = answer_map(answers)

    sections.each_with_index.map do |sec, si|
      sec.merge("questions" => (sec["questions"] || []).map do |q|
        q.merge("your_answer_index" => given[[ si, q["id"].to_i ]])
      end)
    end
  end
  ...
  # { [section_index, question_id] => answer_index }
  def answer_map(answers)
    answers.each_with_object({}) do |a, h|
